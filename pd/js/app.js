...
(function () {
  ...
  const GEN = window.PD_GEN;
  const SHEETS = window.RULE_SHEETS || [];
  ...
  function renderQuiz() {
    ...
    quiz.tInt = setInterval(() => {
      ...
      if (isExam) {
        ...
        if (pace) {
          const expect = (quiz.idx + el / 90) * 55;      // 判断推理约 55 秒/题
  ...
  function choose(i) {
    ...
    if (!ok) {
      store.wrong.push({
        ...
        correct: String(q.options[q.ansIdx].text ?? '见解析').replace(/^图形$/, '（见解析配图）'),
        yours: String(q.options[i].text ?? '—').replace(/^图形$/, '（见解析配图）'),
        analysis: q.analysis,
      });
    ...
    const durTxt = dur < 20 ? `用时 ${dur.toFixed(1)} 秒` : dur > 55 ? `用时 ${fmtSec(dur)}（判断推理建议 55 秒/题内解决，纠结就先标记跳过）` : `用时 ${dur.toFixed(1)} 秒`;
  ...
  function renderHome() {
    ...
    view.innerHTML = `
      <div class="hero">
        <h1>判断推理，<span class="grad">规律先行</span>。</h1>
        <p>行测判断推理六章方法课 —— 图形、定义、类比、翻译、真假与论证，每章讲透套路，配「巩固 / 真题」两档练习。</p>
      </div>
      <div class="ov-card">
        ...
        <div class="ov-mid">
          ...
          <div class="s">${seen ? `已练 <b>${seen}</b> 题，答对 <b>${right}</b> 题` : '从第一章「图形推理」开始——掌握五大规律谱系，图形题就不再靠玄学'}${store.wrong.length ? ` · 错题 <b>${store.wrong.length}</b> 道（<a href="#/wrong">回顾</a>）` : ''}</div>
      ...
      <div class="chapters">
        <div class="ch-row" data-act="nav" data-h="#/cards">
          <div class="ic">📇</div>
          <div><div class="tt">规则速查卡</div><div class="ds">翻译推理规则、图形规律清单、类比关系谱系、论证力度表——考前最后过一遍</div></div>
  ...
  function renderCards() {
    ...
    view.innerHTML = `
      <div class="crumbs" data-act="nav" data-h="#/">← 返回课程表</div>
      <div class="ch-head"><h1>📇 规则速查卡</h1></div>
