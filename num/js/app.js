...
(function () {
  ...
  const GEN = window.SUSUAN_NUMGEN;
  const SHEETS = window.FORMULA_SHEETS || [];
  ...
  function renderQuiz() {
    ...
    view.innerHTML = `
      ...
      <div class="qcard" id="qcard">
        ...
        <div class="opts">${opts.map((o) => `
          <button class="opt" data-i="${o.i}"><span class="k">${o.k}</span><span class="t">${esc(o.text)}</span></button>`).join('')}
        </div>
        <div id="afeed"></div>
      </div>
    ...
    quiz.tInt = setInterval(() => {
      ...
      if (isExam) {
        ...
        if (pace) {
          const expect = (quiz.idx + el / 90) * 60;      // 数量关系约 60 秒/题
  ...
  function choose(i) {
    ...
    const durTxt = dur < 20 ? `用时 ${dur.toFixed(1)} 秒` : dur > 60 ? `用时 ${fmtSec(dur)}（数量关系建议 60 秒/题内解决，不行就先标记跳过）` : `用时 ${dur.toFixed(1)} 秒`;
  ...
  function renderHome() {
    ...
    view.innerHTML = `
      <div class="hero">
        <h1>数量关系，<span class="grad">认题型、套方法</span>。</h1>
        <p>行测数量关系八章方法课 —— 每章讲透一类套路，配「巩固 / 真题」两档练习，认出母题就是半分钟的事。</p>
      </div>
      <div class="ov-card">
        ...
        <div class="ov-mid">
          ...
          <div class="s">${seen ? `已练 <b>${seen}</b> 题，答对 <b>${right}</b> 题` : '从第一章「代入排除法」开始——它是数量关系的万能保底'}${store.wrong.length ? ` · 错题 <b>${store.wrong.length}</b> 道（<a href="#/wrong">回顾</a>）` : ''}</div>
          <div class="bars">${bars}</div>
        </div>
        <a class="cta-exam" href="#/exam" data-act="nav" data-h="#/exam">🎯 综合模考<small>10 题混编 · 真题难度 · 建议 10 分钟</small></a>
      </div>
      ...
      <div class="chapters">
        <div class="ch-row" data-act="nav" data-h="#/cards">
          <div class="ic">📇</div>
          <div><div class="tt">公式速查卡</div><div class="ds">行程、利润、排列组合、几何容斥——考场前最后过一遍的公式清单</div></div>
  ...
  function renderCards() {
    ...
    view.innerHTML = `
      <div class="crumbs" data-act="nav" data-h="#/">← 返回课程表</div>
      <div class="ch-head"><h1>📇 公式速查卡</h1></div>
      ...
      <p class="mini" style="margin-top:18px">考前 5 分钟把这几张卡过一遍，比多刷十道新题更有用。</p>`;
  }
